    ...
    if sort_order == 'alphabetical'
      if connection.adapter_name == 'SQLite'
        ...
        order(Arel.sql("CASE
          ...
          WHEN substr(category_name, 1, 1) GLOB '[ａ-ｚ]*' THEN 3
          WHEN substr(category_name, 1, 1) GLOB '[A-Z]*' THEN 4
          WHEN substr(category_name, 1, 1) GLOB '[Ａ-Ｚ]*' THEN 5
          WHEN substr(category_name, 1, 1) GLOB '[ｱ-ﾝ]*' THEN 6
        ...
        # MySQL用のSQL
        order(Arel.sql("CASE
          WHEN category_name REGEXP '^[0-9０-９]' THEN 1
          WHEN category_name REGEXP '^[a-z]' THEN 2
          WHEN category_name REGEXP '^[ａ-ｚ]' THEN 3
          WHEN category_name REGEXP '^[A-Z]' THEN 4
          WHEN category_name REGEXP '^[Ａ-Ｚ]' THEN 5
          WHEN category_name REGEXP '^[ｱ-ﾝ]' THEN 6
